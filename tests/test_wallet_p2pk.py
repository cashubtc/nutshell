...
async def test_p2pk_multisig_2_of_2(wallet1: Wallet, wallet2: Wallet):
    ...
    # p2pk test
    secret_lock = await wallet1.create_p2pk_lock(
        pubkey_wallet2, tags=Tags(__root__=[["pubkey", pubkey_wallet1]]), n_sigs=2
    )
...
async def test_p2pk_multisig_quorum_not_met_2_of_3(wallet1: Wallet, wallet2: Wallet):
    ...
    # p2pk test
    secret_lock = await wallet1.create_p2pk_lock(
        pubkey_wallet2, tags=Tags(__root__=[["pubkey", pubkey_wallet1]]), n_sigs=3
    )
...
async def test_p2pk_multisig_with_duplicate_publickey(wallet1: Wallet, wallet2: Wallet):
    ...
    # p2pk test
    secret_lock = await wallet1.create_p2pk_lock(
        pubkey_wallet2, tags=Tags(__root__=[["pubkey", pubkey_wallet2]]), n_sigs=2
    )
    _, send_proofs = await wallet1.split_to_send(
        wallet1.proofs, 8, secret_lock=secret_lock
    )
...
async def test_p2pk_multisig_with_wrong_first_private_key(
    ...
    # p2pk test
    secret_lock = await wallet1.create_p2pk_lock(
        pubkey_wallet2, tags=Tags(__root__=[["pubkey", wrong_public_key_hex]]), n_sigs=2
    )
    _, send_proofs = await wallet1.split_to_send(
        wallet1.proofs, 8, secret_lock=secret_lock
    )
