...
def test_cashu_derivation_path_from_unit():
    assert cashu_unit_derivation_index("sat") == 1967237907
    assert cashu_derivation_path_from_unit("sat", 1) == "m/129372'/1967237907'/1'"


def test_keyset_bip39_cdk_compatible_private_keys():
    keyset = MintKeyset(
        seed=MINT_MNEMONIC,
        derivation_path=cashu_derivation_path_from_unit("sat", 1),
        amounts=[1, 2, 4],
        seed_derivation_method=MINT_KEY_DERIVATION_BIP39,
        version="0.20.0",
    )

    assert keyset.private_keys[1].to_hex() == (
        "80077bcc43a442fa4a3b4a82d8bdde13616d16833b3bc2dc1dd86dafd4ee31ca"
    )
    assert keyset.private_keys[2].to_hex() == (
        "31ed2c09a57a2579f5aef1a486f7e3e378f992d2ef50674e6261216733540629"
    )
    assert keyset.private_keys[4].to_hex() == (
        "374b3ccacb38c860aff640bd85b6ef2067b46a3cc7384fe4b1ba2fe15c361f4c"
    )


def test_mint_mnemonic_validation():
    assert validate_bip39_mnemonic(MINT_MNEMONIC) == MINT_MNEMONIC

    with pytest.raises(ValueError, match="valid BIP39"):
        validate_bip39_mnemonic("not a seed phrase")


def test_mint_key_material_settings_are_exclusive():
    with pytest.raises(Exception, match="MINT_PRIVATE_KEY or MINT_MNEMONIC"):
        Settings(mint_private_key="TEST_PRIVATE_KEY", mint_mnemonic=MINT_MNEMONIC)
...
async def test_keyset_rotation(ledger: Ledger):
    ...
    assert keyset_sat_derivation[:-1] == new_keyset_sat_derivation[:-1], (
...
async def test_keyset_version_detection():
    ...
    assert get_keyset_id_version(v1_keyset.id) == "00", (
    ...
    assert v1_keyset.public_keys is not None
...
async def test_keyset_final_expiry_field():
    ...
    assert keyset_no_expiry.final_expiry is None, (
...
async def test_keyset_v2_deterministic():
    ...
    # With expiry
    final_expiry = 1896187313
    assert keyset1.public_keys is not None
    assert keyset2.public_keys is not None
...
async def test_keyset_backward_compatibility():
    ...
    assert legacy_keyset.public_keys is not None
...
async def test_keyset_versions_produce_correct_id_format():
    ...
    assert keyset_0_15.id.startswith("00"), (
    ...
    assert keyset_0_17.id.startswith("00"), (
    ...
    assert keyset_0_18.id.startswith("00"), (
    ...
    assert keyset_0_20.id.startswith("01"), (
...
async def test_keyset_id_v2_test_vectors():
    ...
    # Verify version byte
    assert get_keyset_id_version(keyset_id_v2_vec1) == "01", (
        "Vector 1 should be version 01"
    )
    assert get_keyset_id_version(keyset_id_v2_vec2) == "01", (
        "Vector 2 should be version 01"
    )
    assert get_keyset_id_version(keyset_id_v2_vec3) == "01", (
