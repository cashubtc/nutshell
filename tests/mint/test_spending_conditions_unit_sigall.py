...
def test_inputs_require_sigall_detects_any_sigall():
    ...
    inputs_only = proof(secret_str(kind=SecretKind.P2PK, data=pub_a, sigflag=SigFlags.SIG_INPUTS))
    sig_all = proof(secret_str(kind=SecretKind.P2PK, data=pub_b, sigflag=SigFlags.SIG_ALL))
...
def test_verify_sigall_spending_conditions_valid():
    ...
    raw_secret = secret_str(kind=SecretKind.P2PK, data=signer_pub, sigflag=SigFlags.SIG_ALL)
...
def test_verify_sigall_spending_conditions_rejects_different_secrets():
    ...
    p1 = proof(secret_str(kind=SecretKind.P2PK, data=pub_a, sigflag=SigFlags.SIG_ALL), [sig_a])
    p2 = proof(secret_str(kind=SecretKind.P2PK, data=pub_b, sigflag=SigFlags.SIG_ALL), [sig_a])
