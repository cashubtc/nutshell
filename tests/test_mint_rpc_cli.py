...
def test_panic_management_cli(cli_prefix):
    ...
    quote_help = runner.invoke(
        cli, [*cli_prefix, "panic-blacklist-mint-quotes", "--help"]
    )
    assert quote_help.exception is None
    assert "QUOTE_IDS" in quote_help.output
    assert "--commit" in quote_help.output
