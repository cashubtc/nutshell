...
async def test_p2pk_multisig_two_signatures_same_pubkey(
    ...
    assert PublicKey(bytes.fromhex(pubkey_wallet2)).schnorr_verify(  # type: ignore[attr-defined]
        msg, bytes.fromhex(proof.p2pksigs[0]), None
    )
    assert PublicKey(bytes.fromhex(pubkey_wallet2)).schnorr_verify(  # type: ignore[attr-defined]
        msg, coincurve_signature, None
    )
