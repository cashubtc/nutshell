"""Nutroot secrets (v3 keysets) crypto core.

Tagged hashes, canonical TLV, leaf serialization, merkle tree, and tweak
math per NUT-10. BIP341's
commitment machinery with Cashu tags and compressed (not x-only) keys.
Byte-identical with cashu-ts src/crypto/nutroot.ts; both are pinned by the
shared vectors in tests/nutroot_v3_vectors.json.
"""
...
NUTROOT_LEAF_TAG = "Cashu_NutrootLeaf"
NUTROOT_BRANCH_TAG = "Cashu_NutrootBranch"
NUTROOT_TWEAK_TAG = "Cashu_NutrootTweak"

NUTROOT_LEAF_VERSION = 0x00

# Leaf type registry (version 0x00). A number means one thing forever.
NUTROOT_LEAF_TYPE: Dict[str, int] = {
    "threshold": 0x01,
    "after": 0x02,
    "hashlock": 0x03,
}
...
def serialize_nutroot_leaf(leaf: NutrootLeaf) -> bytes:
...
def parse_nutroot_leaf(data: bytes) -> NutrootLeaf:
    ...
    if len(data) - 1 > NUTROOT_MAX_LEAF_BYTES:
        raise ValueError(f"Leaf body exceeds {NUTROOT_MAX_LEAF_BYTES} bytes")
    if data[0] != NUTROOT_LEAF_VERSION:
...
def nutroot_leaf_hash(serialized_leaf: bytes) -> bytes:
    ...
    return tagged_hash(NUTROOT_LEAF_TAG, serialized_leaf)


def nutroot_branch_hash(a: bytes, b: bytes) -> bytes:
    """Branch of two child hashes, sorted pair, no left/right flags."""
    lo, hi = (a, b) if a <= b else (b, a)
    return tagged_hash(NUTROOT_BRANCH_TAG, lo, hi)


def nutroot_merkle_root(leaf_hashes: List[bytes]) -> bytes:
    ...
    order cannot invalidate a proof. Slot assignment (NUT-28) still
    ...
    if len(leaf_hashes) > 2**NUTROOT_MAX_TREE_DEPTH:
        raise ValueError(f"Tree exceeds depth {NUTROOT_MAX_TREE_DEPTH}")
    level = sorted(leaf_hashes)
    while len(level) > 1:
        nxt = [
            nutroot_branch_hash(level[i], level[i + 1])
...
def nutroot_merkle_path(leaf_hashes: List[bytes], index: int) -> List[bytes]:
...
def nutroot_root_from_path(leaf_hash: bytes, path: List[bytes]) -> bytes:
    """Recompute a root from a leaf hash and its merkle path."""
    if len(path) > NUTROOT_MAX_TREE_DEPTH:
        raise ValueError(f"Merkle path exceeds depth {NUTROOT_MAX_TREE_DEPTH}")
    if len(leaf_hash) != 32 or any(len(sibling) != 32 for sibling in path):
        raise ValueError("Merkle hashes must be 32 bytes")
    acc = leaf_hash
    for sibling in path:
        acc = nutroot_branch_hash(acc, sibling)
    return acc


def nutroot_tweak(internal_key: PublicKey, merkle_root: Optional[bytes] = None) -> int:
    """Tweak scalar tagged_hash("Cashu_NutrootTweak", K || root) mod n.

    Omit merkle_root for the empty tweak (aggregated keys, NUT-10).
    """
    internal_key_bytes = internal_key.format()
    if merkle_root is not None:
        digest = tagged_hash(NUTROOT_TWEAK_TAG, internal_key_bytes, merkle_root)
    else:
        digest = tagged_hash(NUTROOT_TWEAK_TAG, internal_key_bytes)
    return int.from_bytes(digest, "big") % SECP256K1_N


def nutroot_tweak_pubkey(
    internal_key: PublicKey, merkle_root: Optional[bytes] = None
) -> PublicKey:
    """The v3 secret P = K + t*G."""
    t = nutroot_tweak(internal_key, merkle_root)
    return internal_key.add(t.to_bytes(32, "big"))


def nutroot_tweak_seckey(
...
def verify_nutroot_commitment(
    ...
    """
    root = nutroot_root_from_path(nutroot_leaf_hash(serialized_leaf), merkle_path)
...
def is_nutroot_point_secret(secret: str, keyset_id: str) -> bool:
