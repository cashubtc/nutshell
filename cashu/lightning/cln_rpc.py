...
class CLNRPCWallet(LightningBackend):
    ...
    async def pay_invoice(
        ...
        status = PAYMENT_RESULT_MAP.get(data.get("status", ""), PaymentResult.ERROR)
        fee_msat = None
    ...
    async def get_invoice_status(self, checking_id: str) -> PaymentStatus:
        try:
            ...
            return PaymentStatus(
                result=INVOICE_RESULT_MAP.get(status, PaymentStatusResult.ERROR),
            )
        except Exception as exc:
    ...
    async def get_payment_status(self, checking_id: str) -> PaymentStatus:
        try:
            ...
            pay = pays[0]
            result = PAYMENT_STATUS_RESULT_MAP.get(pay.get("status"))
            if result is None:
                return PaymentStatus(
