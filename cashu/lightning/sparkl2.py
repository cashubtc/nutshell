...
class SparkL2Wallet(LightningBackend):
    ...
    async def _ensure_sdk(self):
        ...
        if not settings.mint_spark_mnemonic:
            raise Exception(
                "MINT_SPARK_MNEMONIC is required to initialize SparkL2Wallet. Please set a 12, 15, 18, 21, or 24-word seed phrase in your environment."
            )
        ...
        try:
            self.sdk = await breez_sdk_spark.connect(
                request=breez_sdk_spark.ConnectRequest(
                    config=config, seed=seed, storage_dir=storage_dir
                )
            )
    ...
    async def pay_invoice(
        ...
        if not prepare_res.payment_method.is_bolt11_invoice():
            return PaymentResponse(
                result=PaymentResult.FAILED,
                error_message="Only BOLT11 payments are supported",
            )

        # Ensure fee is within limits before attempting the payment.
        ...
        if total_fee_sats * 1000 > fee_limit_msat:
            return PaymentResponse(
                result=PaymentResult.FAILED,
                error_message=(
                    f"Fee estimate ({total_fee_sats} sats) exceeds limit "
                    f"({fee_limit_msat // 1000} sats)"
                ),
            )
        ...
        try:
            ...
            return PaymentResponse(
                result=PaymentResult.FAILED,
                error_message=f"Failed to build payment request: {str(e)}",
            )
    ...
    async def get_payment_quote(
        ...
        try:
            ...
            if not prepare_res.payment_method.is_bolt11_invoice():
                raise Exception("Only BOLT11 payments are supported")
            ...
            if self.unit == Unit.sat:
                fee_amount = Amount(
                    Unit.sat, fee_amount.to(Unit.sat, round="up").amount
                )
                amount_unit = Amount(
                    Unit.sat, amount_unit.to(Unit.sat, round="up").amount
                )
