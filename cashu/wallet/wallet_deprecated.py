...
class LedgerAPIDeprecated(SupportsHttpxClient, SupportsMintURL):
    ...
    async def _get_info_deprecated(self) -> GetInfoResponse:
        ...
        mint_info = GetInfoResponse(
            **mint_info_deprecated.model_dump(exclude={"parameter", "nuts", "contact"})
        )
        # monkeypatch nuts
        mint_info.nuts = {}
        return mint_info
    ...
    async def mint_deprecated(
        ...
        """
        outputs_deprecated = [BlindedMessage_Deprecated(**o.model_dump()) for o in outputs]
    ...
    async def melt_deprecated(
        ...
        resp = await self.httpx.post(
            f"{self.url}/melt",
            json=payload.model_dump(include=_meltrequest_include_fields(proofs)),  # type: ignore
        )
        self.raise_on_error(resp)
        return_dict = resp.json()
    ...
    async def split_deprecated(
        ...
        mint_response = PostSwapResponse_Deprecated.model_validate(promises_dict)
    ...
    async def check_proof_state_deprecated(
        ...
        resp = await self.httpx.post(
            join(self.url, "/check"),
            json=payload.model_dump(include=_check_proof_state_include_fields(proofs)),  # type: ignore
        )
        self.raise_on_error(resp)
    ...
    async def restore_promises_deprecated(
        ...
        resp = await self.httpx.post(join(self.url, "/restore"), json=payload.model_dump())
        self.raise_on_error(resp)
        response_dict = resp.json()
