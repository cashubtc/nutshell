...
class LedgerCrudSqlite(LedgerCrud):
    ...
    async def update_melt_quote(
        ...
        *,
        ...
        await (conn or db).execute(
            f"""
            UPDATE {db.table_with_schema("melt_quotes")} SET state = :state, fee_paid = :fee_paid, paid_time = :paid_time, proof = :proof, checking_id = :checking_id, method_data = :method_data, amountless_msat = :amountless_msat WHERE quote = :quote
            """,
            {
                "state": quote.state.value,
                "fee_paid": quote.fee_paid,
