...
async def migrate(no_dry_run):
    ...
    assert settings.mint_private_key, (
        "MINT_PRIVATE_KEY is required for this migration. MINT_MNEMONIC is not supported here."
    )
    assert len(settings.mint_seed_decryption_key) > 12, (
