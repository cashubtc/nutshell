...
@cli.command(
    "panic-blacklist-mint-quotes",
    help="Resolve mint quote IDs to issued promises; use --commit to store them.",
)
@click.argument("quote_ids", nargs=-1, required=True)
@click.option("--reason", required=True)
@click.option("--operator", default="")
@click.option("--commit", is_flag=True)
@click.pass_context
def panic_blacklist_mint_quotes(
    ctx: Context,
    quote_ids: tuple[str, ...],
    reason: str,
    operator: str,
    ...
    request = management_pb2.PanicMintQuoteIdsRequest(  # type: ignore[attr-defined]
        quote_ids=list(quote_ids), reason=reason, operator=operator
    )
    method = (
        ctx.obj["STUB"].CommitPanicMintQuotes
        if commit
        else ctx.obj["STUB"].ResolvePanicMintQuotes
    )
    click.echo(method(request))
